...
//*****************************************************************************
// writeCommand() sends a command byte c to the OLED via SPI.
void writeCommand(unsigned char c)
{
    unsigned long ulDummy;

    // Set CS (Pin 61) to low to start sending data
    MAP_SPICSEnable(GSPI_BASE);
    GPIOPinWrite(GPIOA0_BASE, 0x40, 0x00);

    // Set DC (Pin 62) to low to write command
    ...
    // Set CS (Pin 61) to high to end sending data
    MAP_SPICSDisable(GSPI_BASE);
    GPIOPinWrite(GPIOA0_BASE, 0x40, 0x40);
}

//*****************************************************************************
// WriteData() sends a data byte c to the OLED via
void writeData(unsigned char c)
{
    ...
    // Set DC (Pin 62) to high to write data
...
//*****************************************************************************
/* This function assumes that the RESET pin of the
